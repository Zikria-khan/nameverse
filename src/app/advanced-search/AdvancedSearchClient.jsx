...
export default function AdvancedSearchClient() {
  ...
  return (
    <div className="w-full">
                        ...
                        {expandedSections[key] && <div className="space-y-4 pl-2">
                          {category.filters.map(filter => (
                            <div key={filter.id}>
                              ...
                              {filter.type === 'radio' && <div className="flex gap-2 flex-wrap">{filter.options.map(opt => <button key={opt} onClick={() => handleFilterChange(filter.id, opt)} className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition ${activeFilters[filter.id] === opt ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-gray-50 text-gray-700 border-gray-200 hover:bg-gray-100'}`}>{opt}</button>)}</div>}
                              ...
                              {filter.type === 'select' && <select value={activeFilters[filter.id] || ''} onChange={(e) => handleFilterChange(filter.id, e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"><option value="">Select...</option>{filter.options.map(opt => <option key={opt} value={opt}>{opt}</option>)}</select>}
                              {filter.type === 'text' && <input type="text" value={activeFilters[filter.id] || ''} onChange={(e) => handleFilterChange(filter.id, e.target.value)} placeholder={filter.placeholder} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />}
      ...
      <section className="py-12 px-4 sm:px-6 lg:px-8 bg-white/50 border-t border-gray-200">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">Frequently Asked Questions About Name Search</h2>
          <div className="grid md:grid-cols-2 gap-6">
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200"><h3 className="font-bold text-gray-900 mb-2">Can I save my favorite searches?</h3><p className="text-gray-600 text-sm">Yes! Once you find names you love, save them to your personal collection for comparison.</p></div>
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200"><h3 className="font-bold text-gray-900 mb-2">How often is the name database updated?</h3><p className="text-gray-600 text-sm">We update our database monthly with new names and updated popularity rankings.</p></div>
          </div>
        </div>
      </section>
